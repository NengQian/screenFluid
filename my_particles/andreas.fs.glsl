...
uniform vec3 light_color = vec3(0.3, 0.6, 1.0);
uniform vec3 light_dir_eye = vec3(0.0, -1.0, 0.0);
...
void main()
{
    ...
    if (t >= 0)
    {
		...
		// vec4 pixelPos = vec4(vs_in.sphere_pos_view.xyz - normal*vs_in.sphere_radius, 1.0);
	    // float depth =  -pixelPos.z/10.0; // since z here is negative in eyespace...
		// depth = depth*5*10-5;
        float depth = -x.z/10;
        float visDepth = depth;
        FragColor = vec4(visDepth,visDepth,visDepth,1);
